...
Accepted (2026-08-05) by the seotracker maintainer under EVE-33.
...
seotracker can read a project's Google Search Console (GSC) property, but an agent
...
Accept the design proposed in [PR #106](https://github.com/ucsahinn/seotracker/pull/106)
...
The connecting seotracker user owns the Better Auth grant. Better Auth stores its
...
Hosted seotracker reuses its Google OAuth client. A self-hosted operator reuses
...
The `ga4_connections` row belongs to the seotracker project and organization, not
...
custom-metric inputs are excluded from v1. These reports do not consume seotracker
credits.
...
seotracker does not persist report rows in v1. A later cache needs an approved
...
- Acceptance authorizes implementation work. It does not advertise GA4 as an
  available seotracker capability.
...
- [seotracker GSC integration decision](./0003-google-search-console-integration.md)
