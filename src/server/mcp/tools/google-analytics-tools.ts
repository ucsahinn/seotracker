...
export const getGoogleAnalyticsOrganicLandingPagesTool = {
  ...
  config: {
    ...
    description:
      "Read organic-search landing page sessions, engagement, key events, transactions, and revenue from the project's connected GA4 property. Defaults to the last 28 complete property days. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsPagePerformanceTool = {
  ...
  config: {
    ...
    description:
      "Read page views, users, engagement duration, and key events from the connected GA4 property. Organic Search is the default; set channel to all to include every channel. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsKeyEventsTool = {
  ...
  config: {
    ...
    description:
      "Read active GA4 key events with counts and users by event or organic landing page. Previous-period comparison is available for the event breakdown. Read-only and uses no seotracker credits.",
...
export const getSearchOpportunitiesTool = {
  ...
  config: {
    ...
    description:
      "Join Search Console pages ranking in positions 4–20 with GA4 organic landing-page outcomes, then score matched opportunities by demand, business value, and reachability. Unmatched pages remain visible and unscored. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsOrganicOverviewTool = {
  ...
  config: {
    ...
    description:
      "Answer whether organic traffic is improving with top-line sessions, users, engagement, key events, transactions, revenue, an equal-length previous-period comparison, and a daily or weekly trend. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsTrafficAcquisitionTool = {
  ...
  config: {
    ...
    description:
      "Compare session acquisition by channel group, source/medium, or campaign, including sessions, users, engagement, key events, transactions, and revenue. Previous-period comparison is available for channel group; source/medium also reports attribution-quality diagnostics. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsEcommercePerformanceTool = {
  ...
  config: {
    ...
    description:
      "Read item views, add-to-cart units, purchases, and item revenue by item, or transactions and purchase revenue by landing page. Returns a detected, none, or unknown activity state; landing pages can be limited to those with transactions. Organic Search is the default. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsSiteSearchTool = {
  ...
  config: {
    ...
    description:
      "Read measured internal search terms with search events, users, sessions, engaged sessions, and engagement rate. Requires GA4 site-search measurement. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsAudienceBreakdownTool = {
  ...
  config: {
    ...
    description:
      "Read device, country, or new-versus-returning users, sessions, engagement, and key events. Previous-period comparison is available for device and new-versus-returning breakdowns. No demographic or user-level dimensions. Read-only and uses no seotracker credits.",
...
export const getGoogleAnalyticsMeasurementHealthTool = {
  ...
  config: {
    ...
    description:
      "Diagnose the connected property's data streams, web measurement IDs, enhanced-measurement settings, key events, and custom definitions. Read-only and uses no seotracker credits.",
